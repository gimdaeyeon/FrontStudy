...
export default class GameBoard {
    ...
    handleKeyUp(event) {
        ...
        if (event.keyCode === Phaser.Input.Keyboard.KeyCodes.LEFT && this.canMoveBlock(-1, 0)) {
            ...
            this.currentTetrisBlock.move(1, 0);
        } else if (event.keyCode === Phaser.Input.Keyboard.KeyCodes.DOWN && this.canMoveBlock(0, 1)) {
    ...
    render() {
        this.scene.children.removeAll(); // 기존에 그려져있는 이미지들을 지운다
            ...
            for (let j = 0; j < GameConfig.MainScene.GAME_BOARD_WIDTH_CNT; j++) {
                if (this.renderBoard[i][j] == 0) {
                    this.scene.add.image(j * GameConfig.MainScene.RENDER_TILE_SIZE,
                        ...
                        GameConfig.MainScene.BACKGROUND_TILE_SPRITE_SHEET_KEY, 0)
